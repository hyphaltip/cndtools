...
#ifndef __POLYTOPE_COUNTINGPOLYTOPE_HH__
#define __POLYTOPE_COUNTINGPOLYTOPE_HH__
...
namespace polytope {
	...
	public:
		...
		typedef std::vector<Count> CountList;
		...
		const CountList& getVertexCounts() const;
	...
	template<typename T>
	Polytope<T>::Polytope(const VectorList& points,
						  const CountList& counts,
						  bool reduced)
		: ambientDim(points.front().size()),
		  vertices(points),
	...
	template<typename T>
	const typename Polytope<T>::CountList& Polytope<T>::getVertexCounts() const {
		ensureReduced();
		return counts;
	}
	...
	Polytope<T>& Polytope<T>::operator+=(const Polytope& p) {
		if (vertices.empty()) {
			*this = p;
			...
			Count count = p.counts.front();
			
			typename VectorList::iterator vertexPos =
				std::lower_bound(vertices.begin(), vertices.end(), v);
			typename CountList::iterator countPos =
				counts.begin() + std::distance(vertices.begin(), vertexPos);
		...
		if (vertices.empty() or p.vertices.empty()) {
			...
			for (size_t i = 0; i < vertices.size(); ++i) {
				for (size_t j = 0; j < p.vertices.size(); ++j) {
					...
					newCounts.push_back(counts[i] * p.counts[j]);
				}
			}
	...
	Polytope<T>::combineDuplicatePoints() const {
		...
		CountIterator c = counts.begin() + 1, lastC = counts.begin();
		...
		counts.erase(lastC + 1, counts.end());
	}
...
// 	}
	...
	Polytope<T>::removeRedundantPoints2D() const {
		...
		typedef typename VectorList::reverse_iterator RIt;

		std::vector<It> side;
		side.reserve(n);

		// Compute side hull
		side.push_back(vertices.begin());
		side.push_back(vertices.begin() + 1);
		for (It i = vertices.begin() + 2; i != vertices.end(); ++i) {
			side.push_back(i);
			while (side.size() > 2 and
				   not isRightTurn2D(**(side.end() - 3),
									 **(side.end() - 2),
									 **(side.end() - 1))) {
				side.erase(side.end() - 2);
			}
		...
		for (size_t i = 0; i < side.size(); ++i) {
			redundant[side[i] - vertices.begin()] = false;
		}

		side.clear();
		
		// Compute side hull
		side.push_back(vertices.rbegin().base() - 1);
		side.push_back((vertices.rbegin() + 1).base() - 1);
		for (RIt i = vertices.rbegin() + 2; i != vertices.rend(); ++i) {
			side.push_back(i.base() - 1);
			while (side.size() > 2 and
				   not isRightTurn2D(**(side.end() - 3),
									 **(side.end() - 2),
									 **(side.end() - 1))) {
	...
	Polytope<T>::removeRedundantPoints(const std::vector<bool>& redundant) const {
		...
		CountIterator insertC = counts.begin(), currC = counts.begin();
		...
		counts.erase(insertC, counts.end());
	}
	...
	std::ostream& operator<<(std::ostream& strm, const Polytope<T>& p) {
		...
		typedef typename Polytope<T>::CountList::iterator CountIterator;
		
		p.ensureReduced();
		VertexIterator v = p.vertices.begin();
		CountIterator c = p.counts.begin();
...
#endif // __POLYTOPE_COUNTINGPOLYTOPE_HH__
